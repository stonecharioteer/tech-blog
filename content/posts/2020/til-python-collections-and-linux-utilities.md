...
description:
  "Today I learned about Python collections.defaultdict performance benefits,
  the sorted function's reverse parameter, and using arandr as a frontend for
  xrandr in tiling window managers."
tags:
  - til
  - python
  - collections
...
Today I discovered practical optimizations in Python's collections module and
explored Linux display management tools that make working with multiple monitors
much easier in tiling window managers.

## Python Collections Performance

### defaultdict vs dict.get() Performance

`collections.defaultdict` provides better performance than checking `{}.get()`
...
{{< tip title="defaultdict Performance Benefits" >}} **Why defaultdict is faster
than dict.get():**
...
- **Memory efficiency** - Reduces object creation overhead {{< /tip >}}
...
{{< note title="Sorting Best Practices" >}} **When to use sorted() vs
list.sort():**

- **sorted()**: When you need to keep the original list unchanged
- **list.sort()**: When you want to modify the list in-place (more memory
  efficient)
- **reverse=True**: Always prefer this over manual reversal
- **Complex keys**: Use lambda functions or operator.itemgetter for clarity
- **Multiple criteria**: Consider using tuple keys with negation for mixed sort
  orders {{< /note >}}
...
`arandr` provides a visual interface for configuring multiple monitors in tiling
window managers:
...
{{< example title="Monitor Management Workflow" >}} **Typical Multi-Monitor
Setup Process:**
...
6. **Set up hotplug detection** for dynamic configuration {{< /example >}}
...
Today's exploration of `collections.defaultdict` demonstrates how choosing the
right data structure can provide measurable performance improvements:
...
The combination of `arandr` and `xrandr` showcases how GUI tools can complement
...
_Today's learning reinforced that small optimizations in both code and
environment setup can compound to create significant productivity improvements
over time._
